...
class oLocking
{
	...
	function __construct()
	{
		...
		$this->ConfirmAdmin = "O Administrador %s cancelou a sua sessão";
		$this->ConfirmUser = "O tempo de sessão expirou";
		$this->LockAdmin = "O registo foi editado %s durante %s minutos";
		$this->LockUser = "O registo está a ser editado por outro utilizador";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."Desbloquear registo".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."Editar registo".'</a>';
			}
			return $str;
		}
