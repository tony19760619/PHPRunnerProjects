...
class LookupField extends EditControl
{
	...
	public function buildListOfValues($avalue, $value, $mode, $searchOption)
	{
		...
		{ 
			case LCT_DROPDOWN:
				...
				if( !$this->multiselect )
					echo '<option value="">'."Por Favor Selecione".'</option>';
	...
	{		
		...
		if($this->bUseCategory)
		{
			...
			{			
				...
				echo '&nbsp;<a href="#" id="'.$this->openlookup.'"'.$visibility.'>'."Selecionar".'</a>';
			}				
	...
	protected function buildMultiselectAJAXLookup($avalue, $value, $mode, $searchOption)
	{
		...
		if( $this->LCType == LCT_LIST )
			echo $forLinkView.'<a href="#" id="'.$this->openlookup.'">'."Selecionar".'</a>';
		
		if( $this->addnewitem )
			echo $forLinkView."<a href=# id='addnew_".$this->cfield."'>"."Adicionar Novo"."</a>";				
	}
