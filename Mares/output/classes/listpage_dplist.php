...
//			array( "name" => "record_controls_block", "align" => "left" ) 
	...
	protected function getHeaderControlsBlocks()
	{
		...
		if( $this->xt->getVar("saveall_link") )
		{
			...
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button" href="#" '.$savealllink_attrs.' '.$savealllink_span.'>'."Guardar todos".'</a> ';					  
			else
				$buttons.= '<a class="' . $bs_button_class . '" href="#" '.$savealllink_attrs.' '.$savealllink_span.'>'."Guardar todos".'</a> ';					  
		}
		...
		if( $this->deleteAvailable() && $this->xt->getVar("deleteselected_link") )
		{
			...
			// "bs-invisible-button" class need for init hidden in bootstrap
			if( $this->getLayoutVersion() != BOOTSTRAP_LAYOUT) 
				$buttons.= '<a class="rnr-button " href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Apagar".'</a> ';		  
			else
				$buttons.= '<a class="' . $bs_button_class . '" disabled href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Apagar".'</a> ';		  
		}		
