...
//		foreach ($this->googleMapCfg['mainMapIds'] as $mapId)
	...
	{		
		...
		$typeCodeMessage = "Introduza o código exibido";
		$path = GetCaptchaPath();
	...
	*/
	...
	function getProceedLink() 
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "Ir para" . ' '. GetTableCaption( GoodFieldName( $this->tName ) ) 
			. '</a>' 
			. "&nbsp;&nbsp;</span>";
	}
	...
	protected function getDefaultPageTitle($page, $table, $pSet)
	{
		...
		if( $page == "search" )
			return GetTableCaption($table)." - "."Procura Avançada";
		if( $page == "print" )
		...
		if( $page == "register_success" )
			return "Registo realizado com Sucesso";
		if( $page == "changepwd" )
			return "Mudar senha";
		if( $page == "changepwd" )
			return "";
		if( $page == "remind" )
			return "Guardar a minha Password";	
		if( $page == "chart" )
			return GetTableCaption($table); 
		if( $page == "report" )
			return GetTableCaption($table);	
...
*/			
	 ...
	 * @return Array
	 */
	protected function getMenuItemDetailPeersData( $mTName, $mType, $tName, $pType, &$menuRoot )
	{
	...
	protected function getMasterDetailMenuItems( &$menuRoot, &$currentMenuItem )
	{
		...
		while( isset( $_SESSION[ $sessionPrefix."_mastertable" ] ) )
		{
			...
			if( count( $masterTableData ) )
			{
				...
				$otherDetailsData = $this->getMenuItemDetailPeersData( $mTName, $masterTableData["type"], $tName, $pType, $menuRoot );	
