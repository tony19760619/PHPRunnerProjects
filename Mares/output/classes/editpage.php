...
class EditPage extends RunnerPage
{
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."Guardado". " &gt;&gt;&gt;</strong>" );
	}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."O Registo NÃO foi editado"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "O Registo NÃO foi editado".". ".$message;
	...
	public static function EditPageFactory( $params ) {
		...
		if( $params["selection"] && is_array( $params["selection"] ) )
		{
			...
			{
				//php: first index 0; asp: first index 1
				foreach($params["selection"] as $val)
					$str = $val;
				$arr = explode("&", refine($str));
				$i=0;
				foreach($params["keys"] as $key=>$val)
				{
					$params["keys"][$key] = $arr[$i];
					$i++;
				}
			}
		}
