...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("offeraride" == $this->xml_array['tables'][0])
			$isDB = true;
		if("offeraridemessages" == $this->xml_array['tables'][0])
			$isDB = true;
		if("findaridemessages" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lookups" == $this->xml_array['tables'][0])
			$isDB = true;
		if("users" == $this->xml_array['tables'][0])
			$isDB = true;
		if("findaride" == $this->xml_array['tables'][0])
			$isDB = true;
		if("countries" == $this->xml_array['tables'][0])
			$isDB = true;
		if("users" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
