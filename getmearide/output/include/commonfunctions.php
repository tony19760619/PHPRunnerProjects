...
function checkTableName($shortTName, $type=false)
{
	...
	if ("offeraride" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("offeraridemessages" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("findaridemessages" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lookups" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("findaride" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("countries" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	$arr = array();
		$arr[]="offeraride";
		$arr[]="offeraridemessages";
		$arr[]="findaridemessages";
		$arr[]="lookups";
		$arr[]="users";
		$arr[]="findaride";
		$arr[]="countries";
...
//	default permissions
	if($table=="offeraridemessages")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
//	default permissions
	if($table=="findaridemessages")
	{
		// grant all by default
		return "ADESPI".$extraPerm;
	}
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password, &$pageObject = null)
{
	...
	$_SESSION["_offeraride_OwnerID"] = $data["Active"];
