...
function fileCustomExpression($file, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="offeraride" && $field=="TransportPicture")
	{
		;
		return $value;
	}
				if($table=="findaride" && $field=="TransportOfWhatPicture")
	{
		;
		return $value;
	}
...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="offeraride" && $field=="TransportType")
	{
		return " Type = 'Transport Type' ";
	}
				if($table=="offeraride" && $field=="WhoDoesTheDriving")
	{
		return " Type = 'Who Does The Driving' ";
	}
				if($table=="offeraridemessages" && $field=="SentOrReceived")
	{
		return " Type = 'SendType' ";
	}
				if($table=="findaridemessages" && $field=="SentOrReceived")
	{
		return " Type = 'SendType' ";
	}
				if($table=="users" && $field=="UserType")
	{
		return " Type = 'User Type' ";
	}
				if($table=="findaride" && $field=="TypeOfTransport")
	{
		return " Type in ('Transport Type', 'Transport Type Combo') ";
	}
				if($table=="admin_users" && $field=="UserType")
	{
		return " Type = 'User Type' ";
	}
	return "";
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="offeraride" && $field=="Username")
	{
		return $_SESSION["UserID"];
	}
				if($table=="findaridemessages" && $field=="FromUsername")
	{
		return $_SESSION["UserID"];
	}
				if($table=="findaride" && $field=="Username")
	{
		return $_SESSION["UserID"];
	}
	return "";
