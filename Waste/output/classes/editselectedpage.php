...
class EditSelectedPage extends EditPage
{
	...
	*/
	...
	{
		$this->jsSettings["tableSettings"][ $this->tName ]["selection"] = $this->selection;
	...
	public function fillControlFlags( $field, $required = false )
	{
		...
		$data = $this->getCurrentRecordInternal();
		$checked = isset( $data[$field] ) ? "checked" : '';
		$checkbox = "<input type=checkbox class=\"bs-updselbox\" id=updsel_".$gf.$this->id." ".$checked." data-field=\"".runner_htmlspecialchars( $field ) ."\">";
		$label = array();
