...
function checkTableName($shortTName, $type=false)
{
	...
	if ("jobsjobs" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("jobsjobtype" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("jobsstate" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("jobsusers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("jobsjobs");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="jobsjobs";
	}
	$strPerm = GetUserPermissions("jobsjobtype");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="jobsjobtype";
	}
	$strPerm = GetUserPermissions("jobsstate");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="jobsstate";
	}
	$strPerm = GetUserPermissions("jobsusers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="jobsusers";
	}
...
function GetUserPermissionsStatic( $table )
{
	...
	if($table=="jobsjobs" && $sUserGroup=="<Guest>")
	{
			return "SP".$extraPerm;
	}
	if($table=="jobsjobs" && $sUserGroup=="admin")
	{
...
//	default permissions	
	...
	if($table=="jobsjobtype" && $sUserGroup=="<Guest>")
	{
			return "".$extraPerm;
	}
	if($table=="jobsjobtype" && $sUserGroup=="admin")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	if($table=="jobsjobtype")
	{
		return "".$extraPerm;
	}
	if($table=="jobsstate" && $sUserGroup=="<Guest>")
	{
			return "".$extraPerm;
	}
	if($table=="jobsstate" && $sUserGroup=="admin")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
	...
	if($table=="jobsusers" && $sUserGroup=="<Guest>")
	{
			return "".$extraPerm;
	}
	if($table=="jobsusers" && $sUserGroup=="admin")
	{
			return "AEDSPI".$extraPerm;
	}
//	default permissions	
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("jobsusers")
...
function SetAuthSessionData($pUsername, &$data, $fromFacebook, $password, &$pageObject = null)
{
	...
	$_SESSION["_jobsjobs_OwnerID"] = $data["Email"];
