...
function checkTableName($shortTName, $type=false)
{
	...
	if ("evcategories" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("evtellfriend" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("evusers" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("evevents" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("evcategories");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="evcategories";
	}
	$strPerm = GetUserPermissions("evtellfriend");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="evtellfriend";
	}
	$strPerm = GetUserPermissions("evusers");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="evusers";
	}
	$strPerm = GetUserPermissions("evevents");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="evevents";
	}
...
function GetUserPermissionsStatic( $table )
{
	...
	if($table=="evcategories" && $sUserGroup=="<Guest>")
	{
			return "".$extraPerm;
	}
//	default permissions	
	if($table=="evcategories")
	{
		// grant all by default
		return "ADESPI".$extraPerm;	
	}
	if($table=="evtellfriend" && $sUserGroup=="<Guest>")
	{
			return "A".$extraPerm;
	}
//	default permissions	
	if($table=="evtellfriend")
	{
		// grant all by default
		return "ADESPI".$extraPerm;	
	}
	if($table=="evusers" && $sUserGroup=="<Guest>")
	{
			return "".$extraPerm;
	}
//	default permissions	
	...
	if($table=="evevents" && $sUserGroup=="<Guest>")
	{
			return "S".$extraPerm;
	}
//	default permissions	
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("evusers")
