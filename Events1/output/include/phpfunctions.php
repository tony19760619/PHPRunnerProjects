...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="evevents" && $field=="Subject")
	{
		$value="<a href='evevents_view.php?editid1=".$data["EventID"]."'>".$data["Subject"]."</a>";;
		return $value;
	}	
				if($table=="evevents" && $field=="ToTime")
	{
...
;
		return $value;
				...
				if($table=="evevents" && $field=="month_year")
	{
		global $locale_info;
$arr = explode(" ",$data["month_year"]);
$value =  $locale_info["LOCALE_SMONTHNAME".(int)$arr[1]].' '.$arr[0];;
		return $value;
	}	
	return $value;
}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="evtellfriend" && $field=="EmailSubject") 
	{
		return "You have been notified about \"" . $_SESSION["EventsName"] . "\" on " . format_longdate(db2time(now()));
	}
				if($table=="evtellfriend" && $field=="EmailBody") 
	{
		return $_SESSION["Body"];
	}
				if($table=="evevents" && $field=="FromDate") 
	{
		return now();
	}
				if($table=="evevents" && $field=="ToDate") 
	{
		return now();
	}
	return "";
