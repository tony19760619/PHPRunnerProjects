...
$page_layouts["waste_search"] = $layout;
...
$mode = SEARCH_SIMPLE;
$templatefile = GetTemplateName("waste", "search");
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if($pageObject->pSet->getLookupTable("WasteDescr"))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][$pageObject->pSet->getLookupTable("WasteDescr")] = GetTableURL($pageObject->pSet->getLookupTable("WasteDescr"));
	
	$pageObject->fillFieldToolTips("WasteDescr");	
	
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("WasteDescr");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "WasteDescr";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = $pageObject->pSet->getDefaultValue( "WasteDescr" );
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
		$firstFieldParams['not'] = false;
	}
	...
	if(isEnableSection508())
		$xt->assign_section("WasteDescr_label","<label for=\"".GetInputElementId("WasteDescr", $id, PAGE_SEARCH)."\">","</label>");
	else 
		$xt->assign("WasteDescr_label", true);
	
	$xt->assign("WasteDescr_fieldblock", true);
	$xt->assignbyref("WasteDescr_editcontrol", $ctrlBlockArr['searchcontrol']);
	$xt->assign("WasteDescr_notbox", $ctrlBlockArr['notbox']);
	// create second control, if need it
	$xt->assignbyref("WasteDescr_editcontrol1", $ctrlBlockArr['searchcontrol1']);
	// create search type select
	$xt->assign("searchtype_WasteDescr", $ctrlBlockArr['searchtype']);
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("WasteDescr");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl)
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"WasteDescr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"WasteDescr", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
