...
{
global $conn;
$strSQLExists = "select * from Company where company_id=1";
...
{
	// if record exists do something
  $strs = explode("<br>",$data['seller_info']);
  foreach($strs as $str)
  {
    $ret =  $str."\n".@$ret;
  }
...
{
	// if dont exist do something else
  return "General Manager: \n".  "Street Address: \n". "Area Code: \n\n". "Phone: \n". "Website: www.website.com\n". "Email Address: manager@store.com";
}
}
function GetDefaultCompanyTAX()
{
global $conn;
$strSQLExists = "select * from Company where company_id=1";
$rsExists = db_query($strSQLExists,$conn);
...
function GetDefaultCompanyInvoiceName()
{
global $conn;
$strSQLExists = "select * from Company where company_id=1";
$rsExists = db_query($strSQLExists,$conn);
...
function GetDefaultCompanyTerms()
{
global $conn;
$strSQLExists = "select * from Company where company_id=1";
$rsExists = db_query($strSQLExists,$conn);
...
{
	// if dont exist do something else
  return "Please pay your bill on receipt of invoice.";
}
}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="customerwaste" && $field=="UserId") 
	{
