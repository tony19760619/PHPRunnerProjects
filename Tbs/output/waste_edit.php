...
$page_layouts["waste_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("waste", "inline_edit");
else 
	$templatefile = GetTemplateName("waste", "edit");
...
//	processing WasteDescr - begin
	$condition = 1;

	if($condition)
	{
		$control_WasteDescr = $pageObject->getControl("WasteDescr", $id);
		$control_WasteDescr->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("waste", $pageObject->getPageType(), $keyGetQ);
...
if($readevalues)
{
	$data["WasteDescr"] = $evalues["WasteDescr"];
}
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("WasteDescr_label","<label for=\"".GetInputElementId("WasteDescr", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("waste", "view", $viewlink)."'\"");
