...
$page_layouts["customerwaste_add"] = $layout;
...
{
	$templatefile = GetTemplateName("customerwaste", "inline_add");
}
else
{
	$templatefile = GetTemplateName("customerwaste", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("customerwaste", "add");
...
//	processing DateTime - start
	...
	if($inlineAddOption)
	{
		...
		$control_DateTime->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("customerwaste", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("customerwaste", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("customerwaste", $pageObject->getPageType());
...
{
	$defvalues["UserId"] = GetDefaultValue("UserId", PAGE_ADD);
}
...
//	show readonly fields
	$pageObject->readOnlyFields["UserId"] = $pageObject->showDBValue("UserId", $defvalues);
$linkdata="";
...
//	UserId
	...
	if($display)
	{	
		$showValues["UserId"] = $pageObject->showDBValue("UserId", $data, $keylink);
		$showFields[] = "UserId";
	}	
//	WasteId
	$display = false;
...
//	DateTime
	...
	if($display)
	{	
		$showValues["DateTime"] = $pageObject->showDBValue("DateTime", $data, $keylink);
		$showFields[] = "DateTime";
	}	
//	Amount
	$display = false;
