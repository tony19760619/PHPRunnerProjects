...
$page_layouts["customerwastebyweek_add"] = $layout;
...
{
	$templatefile = GetTemplateName("customerwastebyweek", "inline_add");
}
else
{
	$templatefile = GetTemplateName("customerwastebyweek", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("customerwastebyweek", "add");
...
//	processing Year - end
//	processing Week - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Week = $pageObject->getControl("Week", $id);
		$control_Week->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing Week - end
...
//	processing Week - start
				...
				if($inlineAddOption)
				{
					$control_Week->afterSuccessfulSave();
				}
//	processing Week - end
//	processing Amount - start
				$inlineAddOption = true;
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebyweek", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebyweek", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("customerwastebyweek", $pageObject->getPageType());
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Week_label","<label for=\"".GetInputElementId("Week", $id, PAGE_ADD)."\">","</label>");
...
//	Week
	...
	if($display)
	{	
		$showValues["Week"] = $pageObject->showDBValue("Week", $data, $keylink);
		$showFields[] = "Week";
	}	
//	Amount
	$display = false;
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "customerwastebyweek";
		$options['firstTime'] = 1;
