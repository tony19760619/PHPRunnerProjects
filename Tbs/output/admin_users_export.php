...
$pageObject->display(GetTemplateName("admin_users", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["UserType"] = $pageObject->getExportValue("UserType", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["UserType"] = $pageObject->getViewControl("UserType")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["UserType"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Id")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","UserType")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Username")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Password")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","CustomerId")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Name")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Active")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("admin_users","Email")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("admin_users","Id")."</td>";
		echo "<td>".GetFieldLabel("admin_users","UserType")."</td>";
		echo "<td>".GetFieldLabel("admin_users","Username")."</td>";
		echo "<td>".GetFieldLabel("admin_users","Password")."</td>";
		echo "<td>".GetFieldLabel("admin_users","CustomerId")."</td>";
		echo "<td>".GetFieldLabel("admin_users","Name")."</td>";
		echo "<td>".GetFieldLabel("admin_users","Active")."</td>";
		echo "<td>".GetFieldLabel("admin_users","Email")."</td>";
	}
	echo "</tr>";
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo $values["CustomerId"];//echo runner_htmlspecialchars($values["CustomerId"]); commented for bug #6823
