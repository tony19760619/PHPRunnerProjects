...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("admin_users", "inline_edit");
else 
	$templatefile = GetTemplateName("admin_users", "edit");
...
//	processing UserType - begin
	$condition = 1;

	if($condition)
	{
		$control_UserType = $pageObject->getControl("UserType", $id);
		$control_UserType->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("admin_users", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("admin_users", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["UserType"] = $evalues["UserType"];
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("UserType_label","<label for=\"".GetInputElementId("UserType", $id, PAGE_EDIT)."\">","</label>");
...
//	UserType - 
	$value = $pageObject->showDBValue("UserType", $data, $keylink);
	$showValues["UserType"] = $value;
	$showFields[] = "UserType";
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("admin_users", "view", $viewlink)."'\"");
