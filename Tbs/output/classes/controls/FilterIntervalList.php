...
class FilterIntervalList extends FilterControl
{
	...
	protected function getTotalString($caseCondition, $idx)
	{
		...
		$totalString = $this->aggregate."(".$caseStatement.") as ".AddFieldWrappers($this->fName.$idx)." ";
			
		if( $this->useTotals && $this->totalsfName != $this->fName )
		{
			$caseStatement = $this->getCaseStatement($caseCondition, $fullTotalFieldName, "null");
			$totalString.= ", ". $this->aggregate."(".$caseStatement.") as ".AddFieldWrappers("TOTAL".$idx);
		}		
		return $totalString;
	}	
	...
	{
		return FilterIntervalList::getIntervalFilterWhere($this->fName, $interval, $this->pSet, $this->cipherer, $this->tName);
	}
	 ...
	 * @return String
	 */ 
	static function getIntervalFilterWhere($fName, $intervalData, $pSet, $cipherer, $tableName)
	{
		$fullFieldName = $pSet->getFullNameField($fName); 
		if( $intervalData["remainder"] )  
		{
			...
			foreach($intervalsData as $intervalData)
			{
				...
				$conditions[] = FilterIntervalList::getLimitsConditions($fName, $fullFieldName, $intervalData, $cipherer, $tableName, true);
			}
		...
		return FilterIntervalList::getLimitsConditions($fName, $fullFieldName, $intervalData, $cipherer, $tableName);
	 ...
	 * @return String
	 */ 	
	static function getLimitsConditions($fName, $fullFieldName, $intervalData, $cipherer, $tableName, $inverted = false) 
	{
		...
		if($signLow)
		{
			...
			if( $intervalData["caseSensetive"] )
				...
				$condition[] = db_upper($fValue).$signLow.db_upper($fullFieldName);
		}
		...
		if($signUp)
		{
			...
			if( $intervalData["caseSensetive"] )
				...
				$condition[] = db_upper($fullFieldName).$signUp.db_upper($fValue);
		}
