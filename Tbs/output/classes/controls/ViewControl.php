...
class ViewControl
{
	...
	*/
	...
	protected function getShorteningTextAndMoreLink($value, $cNumberOfChars, $keylink, $isLookup)
	{
		...
		$params = 'pagetype='.$this->container->pSet->_viewPage.'&table='.GetTableURL($strTableName).'&field='.rawurlencode($this->field).$keylink;
		if( $isLookup )
		{
	...
	protected function checkIfLookupValueIsToProcess()
	{
		...
		if( ($pSet->getLookupType($this->field) == LT_LOOKUPTABLE || $pSet->getLookupType($this->field) == LT_QUERY) &&
			$pSet->getLWLinkField($this->field) == $pSet->getLWDisplayField($this->field) && $pSet->multiSelect($this->field) ) 
		{
			return true;
		}
