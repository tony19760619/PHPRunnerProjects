...
class CrossTableReport
{
	...
	protected function fillSessionVariables()
	{
		if( postvalue("group_func") != "" )
			$_SESSION[$this->shortTableName."_group_func"] = postvalue("group_func");
			
		if( postvalue("field") != "" )
			$_SESSION[$this->shortTableName."_field"] = postvalue("field");
			
		if( postvalue("axis_x") != "" )
			$_SESSION[$this->shortTableName."_gr_x"] = postvalue("axis_x");
			
		if( postvalue("axis_y") != "" )
			$_SESSION[$this->shortTableName."_gr_y"] = postvalue("axis_y");
			
		if( postvalue("rname") != "" )
			$_SESSION[$this->shortTableName."_rname"] = postvalue("rname");		
	}
	...
	{
		$gr_x = $_SESSION[$this->shortTableName."_gr_x"];
		$gr_y = $_SESSION[$this->shortTableName."_gr_y"];
	...
	protected function getstrSQL( $strSQL )
	{
		...
		if( $fName != " " )
		{
			$select_field = $this->dataGroupFunction."(".AddFieldWrappers($fName)."), ";
			$this->is_value_empty = false;
			
			$ftype = $this->getFieldType($field);
			if( $this->dataGroupFunction == "avg" && !IsDateFieldType($ftype) ) 
				$avg_func = ", sum(".AddFieldWrappers($fName).") as ".AddFieldWrappers("avg_sum").", count(".AddFieldWrappers($fName).") as ".AddFieldWrappers("avg_count");
			else
		...
		$crtableSQL = $selectClause." from (".$strSQL.") as cross_table".$whereClause." ".$groupByClause." ".$orderByClause;
	...
	protected function getIntervalType($index)
	{
		...
		if( $int_type == 0 ) 
		{
			$wrappedGoodFieldName = AddFieldWrappers( $this->CrossGoodFieldName($field) );
	...
	public function getRadioGroupFunctions()
	{
		$arr = array();
		$arrDisplay = array();
		$res = "";
		...
		for($j = 0; $j < count($arr); $j++)
		{
			...
			if($res == "" || $this->dataGroupFunction == $arr[$j])
				$s = "checked";
			$res.= "<input type=radio value='".$arr[$j]."' name=group_func ".$s." onclick='refresh_crosstable(false);return false;'> ".$arrDisplay[$j]."&nbsp;&nbsp;";
		}
		return $res;
	}
	...
	public function getValuesControl()
	{
		...
		if(count($arr_list) != 0 )
		{
			...
			{
				$s = "";
				if($i == 0 || $i == $_SESSION[$this->shortTableName."_field"])
				{
					$first_field = $i;
					$s = "selected";
				}
	...
	public function getPrintCrossHeader($axis_x,$axis_y,$field,$grfunc)
	{
		$arr_value = array();
