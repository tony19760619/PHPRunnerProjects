...
class RightsPage extends ListPage  
{
	...
	function getRights() 
	{
		$trs = db_query("select `GroupID`,`TableName`,`AccessMask` from `tbs_ugrights` order by `GroupID`", $this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
			$group = $tdata[0];
			$table = $tdata[1];
	...
	**/
	...
	function updateTablePermissions( $table, $group, $mask )
	{
		...
		$sql = "select `AccessMask` from `tbs_ugrights` where `GroupID`=".$group." and `TableName`=".db_prepare_string($table);
		...
		if($data)
		{
			...
			if( strlen($mask) )
			{
				$sql = "update `tbs_ugrights` set `AccessMask`='".$mask."' where `GroupID`=".$group." and `TableName`=".db_prepare_string($table);
			}
			else
			{
				$sql = "delete from `tbs_ugrights` where `GroupID`=".$group." and `TableName`=".db_prepare_string($table);
			}
		}
		else
		{
			if( !strlen($mask) )
				return;
			$sql = "insert into `tbs_ugrights` (`GroupID`, `TableName`, `AccessMask`) values (".$group.", ".db_prepare_string($table).", '".$mask."')";
		}
		$rs = db_exec( $sql, $conn);
	}
}
