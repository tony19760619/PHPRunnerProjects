...
$page_layouts["customerwastebymonth_add"] = $layout;
...
{
	$templatefile = GetTemplateName("customerwastebymonth", "inline_add");
}
else
{
	$templatefile = GetTemplateName("customerwastebymonth", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("customerwastebymonth", "add");
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing WasteId - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_WasteId = $pageObject->getControl("WasteId", $id);
		$control_WasteId->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Year - end
//	processing Month - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_Month = $pageObject->getControl("Month", $id);
		$control_Month->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
...
//	processing Amount - end
	...
	if( $strOriginalTableName == $pageObject->pSet->getOwnerTable("CustomerId") && ( !CheckTablePermissions($strTableName, 'M') || !strlen( $avalues["CustomerId"] ) ) )
		$avalues["CustomerId"] = prepare_for_db("CustomerId", $_SESSION["_".$strTableName."_OwnerID"]);
...
//	insert masterkey value if exists and if not specified
	...
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="waste")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if(!isset($avalues["WasteId"]) || $avalues["WasteId"] == ""){
			$avalues["WasteId"] = prepare_for_db("WasteId",$_SESSION[$sessionPrefix."_masterkey1"]);
		}
...
//	processing Month - start
				...
				if($inlineAddOption)
				{
					$control_Month->afterSuccessfulSave();
				}
//	processing Month - end
//	processing Amount - start
				$inlineAddOption = true;
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebymonth", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebymonth", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("customerwastebymonth", $pageObject->getPageType());
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["CustomerId"]=postvalue("copyid2");
		$copykeys["WasteId"]=postvalue("copyid3");
	}
	else
	{
		$copykeys["Id"]=postvalue("editid1");
		$copykeys["CustomerId"]=postvalue("editid2");
		$copykeys["WasteId"]=postvalue("editid3");
	}
...
//	insert default ownerid value if exists
	if($strOriginalTableName == $pageObject->pSet->getOwnerTable("CustomerId"))
		$defvalues["CustomerId"] = prepare_for_db("CustomerId",$_SESSION["_".$strTableName."_OwnerID"]);
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="waste")
{
	...
	if(postvalue("mainMPageType")<>"add")
		$defvalues["WasteId"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
if($readavalues)
{
	$defvalues["WasteId"]=@$avalues["WasteId"];
	$defvalues["Year"]=@$avalues["Year"];
	$defvalues["Month"]=@$avalues["Month"];
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("WasteId_label","<label for=\"".GetInputElementId("WasteId", $id, PAGE_ADD)."\">","</label>");
	...
	if(isEnableSection508())
		$xt->assign_section("Month_label","<label for=\"".GetInputElementId("Month", $id, PAGE_ADD)."\">","</label>");
...
//	WasteId
	...
	if($display)
	{	
		$showValues["WasteId"] = $pageObject->showDBValue("WasteId", $data, $keylink);
		$showFields[] = "WasteId";
	}	
//	Year
	$display = false;
	...
	if($display)
	{	
		$showValues["Year"] = $pageObject->showDBValue("Year", $data, $keylink);
		$showFields[] = "Year";
	}	
//	Month
	$display = false;
	...
	if($display)
	{	
		$showValues["Month"] = $pageObject->showDBValue("Month", $data, $keylink);
		$showFields[] = "Month";
	}	
//	Amount
	$display = false;
	...
	if($display)
	{	
		$showValues["Amount"] = $pageObject->showDBValue("Amount", $data, $keylink);
		$showFields[] = "Amount";
	}	
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "customerwastebymonth";
		$options['firstTime'] = 1;
