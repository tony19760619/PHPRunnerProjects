...
$page_layouts["customerwaste_detailspreview"] = $layout;
...
if($rowcount) {
	...
	//	DateTime - Short Date
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("DateTime", $data, $keylink);
			$row["DateTime_value"] = $value;
			$format = $pSet->getViewFormat("DateTime");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("DateTime")))
				$class = ' rnr-field-number';
			$row["DateTime_class"] = $class;
	//	Amount - Number
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("customerwaste", "detailspreview"));
