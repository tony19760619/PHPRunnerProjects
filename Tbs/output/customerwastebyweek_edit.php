...
$page_layouts["customerwastebyweek_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("customerwastebyweek", "inline_edit");
else 
	$templatefile = GetTemplateName("customerwastebyweek", "edit");
...
//	processing Year - end
//	processing Week - begin
	$condition = 1;

	if($condition)
	{
		$control_Week = $pageObject->getControl("Week", $id);
		$control_Week->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("customerwastebyweek", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("customerwastebyweek", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Week_label","<label for=\"".GetInputElementId("Week", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("customerwastebyweek", "view", $viewlink)."'\"");
