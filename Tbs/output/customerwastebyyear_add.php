...
add_nocache_headers();
include("include/customerwastebyyear_variables.php");
...
$page_layouts["customerwastebyyear_add"] = $layout;
...
{
	$templatefile = GetTemplateName("customerwastebyyear", "inline_add");
}
else
{
	$templatefile = GetTemplateName("customerwastebyyear", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("customerwastebyyear", "add");
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebyyear", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("customerwastebyyear", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("customerwastebyyear", $pageObject->getPageType());
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "customerwastebyyear";
		$options['firstTime'] = 1;
