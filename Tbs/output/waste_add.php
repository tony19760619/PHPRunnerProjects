...
{
	$templatefile = GetTemplateName("waste", "inline_add");
}
else
{
	$templatefile = GetTemplateName("waste", "add");
}
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing WasteDescr - start
	$inlineAddOption = true;
	if($inlineAddOption)
	{
		$control_WasteDescr = $pageObject->getControl("WasteDescr", $id);
		$control_WasteDescr->readWebValue($avalues, $blobfields, "", false, $afilename_values);
	}
//	processing WasteDescr - end
...
//	processing WasteDescr - start
				...
				if($inlineAddOption)
				{
					$control_WasteDescr->afterSuccessfulSave();
				}
//	processing WasteDescr - end
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("waste", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("waste", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("waste", $pageObject->getPageType());
...
if($readavalues)
{
	$defvalues["WasteDescr"]=@$avalues["WasteDescr"];
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("WasteDescr_label","<label for=\"".GetInputElementId("WasteDescr", $id, PAGE_ADD)."\">","</label>");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$showDetailKeys["customerwaste"]["masterkey1"] = $data["Id"];	
	
	$showDetailKeys["customerwastebymonth"]["masterkey1"] = $data["Id"];	
	
	$showDetailKeys["customerwastebyweek"]["masterkey1"] = $data["Id"];	
	
	$showDetailKeys["customerwastebyday"]["masterkey1"] = $data["Id"];	
	
	$showDetailKeys["customerwastebyyear"]["masterkey1"] = $data["Id"];	
...
//	WasteDescr
	...
	if($display)
	{	
		$showValues["WasteDescr"] = $pageObject->showDBValue("WasteDescr", $data, $keylink);
		$showFields[] = "WasteDescr";
	}	
		$showRawValues["Id"] = substr($data["Id"],0,100);
		$showRawValues["WasteDescr"] = substr($data["WasteDescr"],0,100);
