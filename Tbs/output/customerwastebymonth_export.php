...
$page_layouts["customerwastebymonth_export"] = $layout;
...
$pageObject->display(GetTemplateName("customerwastebymonth", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Year"] = $pageObject->getExportValue("Year", $row);
			$values["Month"] = $pageObject->getExportValue("Month", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["Year"] = $pageObject->getViewControl("Year")->getExportValue($row, "");
			$values["Month"] = $pageObject->getViewControl("Month")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["Month"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","Id")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","CustomerId")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","WasteId")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","Year")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","Month")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("customerwastebymonth","Amount")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("customerwastebymonth","Id")."</td>";
		echo "<td>".GetFieldLabel("customerwastebymonth","CustomerId")."</td>";
		echo "<td>".GetFieldLabel("customerwastebymonth","WasteId")."</td>";
		echo "<td>".GetFieldLabel("customerwastebymonth","Year")."</td>";
		echo "<td>".GetFieldLabel("customerwastebymonth","Month")."</td>";
		echo "<td>".GetFieldLabel("customerwastebymonth","Amount")."</td>";
	}
	echo "</tr>";
