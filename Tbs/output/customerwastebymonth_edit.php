...
$page_layouts["customerwastebymonth_edit"] = $layout;
...
if($inlineedit == EDIT_INLINE)
	$templatefile = GetTemplateName("customerwastebymonth", "inline_edit");
else 
	$templatefile = GetTemplateName("customerwastebymonth", "edit");
...
//	processing Year - end
//	processing Month - begin
	$condition = 1;

	if($condition)
	{
		$control_Month = $pageObject->getControl("Month", $id);
		$control_Month->readWebValue($evalues, $blobfields, $strWhereClause, $oldValuesRead, $efilename_values);
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	HeaderRedirect("customerwastebymonth", $pageObject->getPageType(), $keyGetQ);
...
if(!$data)
{
	...
	{
		HeaderRedirect("customerwastebymonth", "list", "a=return");
		exit();
	}
	else
		$data = array();
...
if($inlineedit != EDIT_INLINE)
{
	...
	if(isEnableSection508())
		$xt->assign_section("Month_label","<label for=\"".GetInputElementId("Month", $id, PAGE_EDIT)."\">","</label>");
...
$xt->assign("viewlink_attrs","id=\"viewButton".$id."\" name=\"viewButton"
	.$id."\" onclick=\"window.location.href='".GetTableLink("customerwastebymonth", "view", $viewlink)."'\"");
