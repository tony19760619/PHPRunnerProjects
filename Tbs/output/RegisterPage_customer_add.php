...
$page_layouts["RegisterPage_customer_add"] = $layout;
...
{
	$templatefile = GetTemplateName("RegisterPage_customer", "inline_add");
}
else
{
	$templatefile = GetTemplateName("RegisterPage_customer", "add");
}
...
if(isset($_REQUEST['afteradd'])){
		HeaderRedirect("RegisterPage_customer", "add");
				...
				if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
				{
					...
					if (count($keys))
					{
						...
						if($pageObject->pSet->hasEditPage() && $permis['edit'])
							$message .="&nbsp;<a href='".GetTableLink("RegisterPage_customer", "edit", $keylink)."'>"."Edit"."</a>&nbsp;";
						if($pageObject->pSet->hasViewPage() && $permis['search'])
							$message .="&nbsp;<a href='".GetTableLink("RegisterPage_customer", "view", $keylink)."'>"."View"."</a>&nbsp;";
					}
					$mesClass = "";	
				}
			}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	HeaderRedirect("RegisterPage_customer", $pageObject->getPageType());
...
if($pageObject->isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_POPUP) && !isMobile())
{
	...
	for($d=0;$d<count($dpParams['ids']);$d++)
	{
		...
		$options['masterTable'] = "RegisterPage_customer";
		$options['firstTime'] = 1;
