...
$page_layouts["waste_export"] = $layout;
...
$pageObject->display(GetTemplateName("waste", "export"));
...
function ExportToXML($rs, $nPageSize, $strTableName, $eventObj, $cipherer, $pageObject)
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["WasteDescr"] = $pageObject->getExportValue("WasteDescr", $row);
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows < $nPageSize) && $row)
	{
		$values = array();
			...
			$values["WasteDescr"] = $pageObject->getViewControl("WasteDescr")->getExportValue($row, "");
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.str_replace('"', '""', $values["WasteDescr"]).'"';
			echo $outstr;
		}
...
// write header
	...
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("waste","Id")).'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel(GetFieldLabel("waste","WasteDescr")).'</td>';	
	}
	else
	{
		echo "<td>".GetFieldLabel("waste","Id")."</td>";
		echo "<td>".GetFieldLabel("waste","WasteDescr")."</td>";
	}
	echo "</tr>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["WasteDescr"]);
					else
						echo $values["WasteDescr"];
			echo '</td>';
			echo "</tr>";
		}
